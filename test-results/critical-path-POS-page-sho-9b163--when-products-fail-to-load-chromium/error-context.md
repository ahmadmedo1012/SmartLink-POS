...
- Name: critical-path.spec.ts >> POS page >> shows error state when products fail to load
- Location: e2e/critical-path.spec.ts:217:7
...
Locator: getByText(/فشل تحميل المنتجات/i)
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByText(/فشل تحميل المنتجات/i)
...
```ts
  ...
  137 |     await expect(page.getByRole("link", { name: /ابدأ الآن مجاناً/ }).first()).toBeVisible()
  138 |   })
  139 | })
  140 | 
